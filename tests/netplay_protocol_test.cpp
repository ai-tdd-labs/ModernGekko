...
constexpr ModernGekkoChunkFn chunk_functions[] = {ChunkA};
...
int main() {
  ...
  if (!server->is_connected)
    return 1;
  std::atomic<bool> stop_lobby_observer = false;
  std::thread lobby_observer([&] {
    while (!stop_lobby_observer.load(std::memory_order_relaxed)) {
  ...
  second.reset();
  first.reset();
  stop_lobby_observer.store(true, std::memory_order_relaxed);
  lobby_observer.join();
  server.reset();
